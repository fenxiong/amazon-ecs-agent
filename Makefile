...
run-integ-tests: test-registry gremlin container-health-check-image run-sudo-tests run-sudo-integ-tests
	ECS_LOGLEVEL=debug ${GOTEST} -tags integration -timeout=25m ./agent/...

run-sudo-tests:
	sudo -E ${GOTEST} -tags sudo -timeout=10m ./agent/...

run-sudo-integ-tests:
	sudo -E ${GOTEST} -tags 'sudo integration' -timeout=10m ./agent/...
