...
const (
	// small backoff value used by unit test - we just want to test retry behavior, not need to wait long
...
func TestSendAttachmentEvent(t *testing.T) {
	...
	handler := NewAttachmentHandler(ctx, statemanager.NewNoopStateManager(), nil, client)
	defer cancel()
...
func TestSendAttachmentEventRetries(t *testing.T) {
	...
	GetSubmitAttachmentStateBackoffFunc = func() retry.Backoff {
		return retry.NewExponentialBackoff(xSubmitStateBackoffMin, xSubmitStateBackoffMax,
			xSubmitStateBackoffJitterMultiple, xSubmitStateBackoffMultiple)
	}
	defer func() {
		GetSubmitAttachmentStateBackoffFunc = GetSubmitAttachmentStateBackoff
	}()
...
func attachmentEvent(attachmentARN string) (api.AttachmentStateChange) {
	return api.AttachmentStateChange{
		Attachment: &apieni.ENIAttachment{
			AttachmentType:   apieni.ENIAttachmentTypeTrunkENI,
			AttachmentARN:    attachmentARN,
