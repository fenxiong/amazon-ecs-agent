...
const (
	// savingStateRetryNum is the number of retries we have when we successfully submitted an attachment
...
func (handler *attachmentHandler) submitAttachmentEvent(attachmentChange *api.AttachmentStateChange) {
	...
	// multiple times (this can happen when udev watch sends multiple attached events for a certain attachment,
	...
	// we need to continue retrying but skip submitting attachment state change when we successfully submitted the state
	// change but failed to save state
