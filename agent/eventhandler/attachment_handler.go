...
func (handler *AttachmentHandler) submitAttachmentEvent(attachmentChange *api.AttachmentStateChange) {
	...
	seelog.Debugf("AttachmentHandler: acquiring lock before sending attachment state change for attachment %s", attachmentARN)
	atmLock.Lock()
	seelog.Debugf("AttachmentHandler: acquired lock for attachment %s", attachmentARN)
