...
type S3ClientCreator interface {
	NewS3ClientForBucket(bucket string, creds credentials.IAMRoleCredentials) (s3client.S3Client, error)
}
...
func (*s3ClientCreator) NewS3ClientForBucket(bucket string,
	...
	cfg := aws.NewConfig().
		...
		WithCredentials(
			awscreds.NewStaticCredentials(creds.AccessKeyID, creds.SecretAccessKey,
				creds.SessionToken)).WithRegion("us-west-2")
	sess := session.Must(session.NewSession(cfg))

	svc := s3.New(sess)
	region, err := getRegionFromBucket(svc, bucket)
	if err != nil {
		return nil, err
	}

	sessWithRegion := session.Must(session.NewSession(cfg.WithRegion(region)))
