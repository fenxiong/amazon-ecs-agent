...
func (engine *DockerStatsEngine) getStatsTaskContainer(task *apitask.Task, dockerContainerName string) {
	var statsTaskContainer *StatsTask
	if task.IsNetworkModeAWSVPC() {
		numberOfContainers := len(task.Containers)
		var taskExists bool
		...
		if !taskExists {
			...
			statsTaskContainer, err = newStatsTaskContainer(task.Arn, containerpid, numberOfContainers, engine.resolver)
			if err != nil {
				return
			}
...
func (engine *DockerStatsEngine) addContainerUnsafe(dockerID string) (*StatsContainer, *StatsTask, error) {
	...
	if !engine.config.DisableMetrics {
		...
		if errResolveContainer == nil {
			engine.getStatsTaskContainer(task, dockerContainer.DockerName)
		}
	}
