...
func (client *APIECSClient) SubmitAttachmentStateChange(change api.AttachmentStateChange) error {
	if change.Attachment == nil { // this should not happen unless there's a bug
		return fmt.Errorf("malformed attachment state change")
	}
	attachmentStatus := change.Attachment.Status.String()

	req := ecs.SubmitAttachmentStateChangeInput{
		Cluster: &client.config.Cluster,
		Attachment: &ecs.AttachmentStateChange{
			AttachmentArn: aws.String(change.Attachment.AttachmentARN),
			Status:        aws.String(attachmentStatus),
		},
	}

	_, err := client.submitStateChangeClient.SubmitAttachmentStateChange(&req)
	if err != nil {
