...
func (task *Task) PostUnmarshalTask(cfg *config.Config,
	...
	// needs the credentials endpoint constructed by the latter.
...
func (task *Task) addEFSVolumes(
	...
	driverOpts := getEFSVolumeDriverOptions(cfg, efsvol, task.GetCredentialsRelativeURI())
...
// GetCredentialsID gets the credentials relative uri  for the task
