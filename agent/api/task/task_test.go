...
func TestPopulateSecrets(t *testing.T) {
	...
	assert.Equal(t, "secretVal4", hostConfig.LogConfig.Config["splunk-token"])
}
...
func TestPopulateSecretsAsEnvOnlySSM(t *testing.T) {
	...
	asmRes1 := &asmsecret.ASMSecretResource{}
	asmRes1.SetCachedSecretValue(asmSecretKeyWest1, "secretValue1")

	asmRes2 := &asmsecret.ASMSecretResource{}
	asmRes2.SetCachedSecretValue(secKeyLogDriver, "secretValue2")
	...
	task.AddResource(asmsecret.ResourceName, asmRes1)
	task.AddResource(asmsecret.ResourceName, asmRes2)
