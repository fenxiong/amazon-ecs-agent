...
func TestGetEFSDriverOptions_UseLocalVolumeDriver(t *testing.T) {
	...
	efsVolCfg := getEFSVolumeConfig()
	credsURI := "/v2/creds-id"
	options := getEFSVolumeDriverOptions(cfg, efsVolCfg, credsURI)
	assert.Equal(t, "nfs", options["type"])
	assert.Equal(t, "addr=fs-12345.efs.us-west-1.amazonaws.com,nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport", options["o"])
	assert.Equal(t, ":/my/root/dir", options["device"])
}
...
func TestGetEFSDriverOptions_UseECSVolumePlugin(t *testing.T) {
	...
	assert.Equal(t, "tls,tlsport=12345,iam,awscredentialsrelativeuri=/v2/creds-id,accesspoint=fsap-123", options["o"])
	assert.Equal(t, "fs-12345:/my/root/dir", options["device"])
}
