...
func TestPostUnmarshalWithFirelensContainer(t *testing.T) {
	...
	assert.Equal(t, 2, len(task.Containers[1].TransitionDependenciesMap[apicontainerstatus.ContainerCreated].ResourceDependencies))
...
func TestPopulateSecretLogOptionsToFirelensContainer(t *testing.T) {
	...
	assert.Equal(t, 2, len(task.Containers[1].Environment))
