...
func (handler *attachInstanceENIHandler) handleSingleMessage(message *ecsacs.AttachInstanceNetworkInterfacesMessage) error {
	...
	if err := validateAttachInstanceNetworkInterfacesMessage(message); err != nil {
		return errors.Wrapf(err,
			"attach instance eni message handler: error validating AttachInstanceNetworkInterfacesMessage received from ACS")
	}
	...
	return handleENIAttachment(apieni.ENIAttachmentTypeTrunkENI, attachmentARN, "", mac, expiresAt, handler.state, handler.saver)
}
