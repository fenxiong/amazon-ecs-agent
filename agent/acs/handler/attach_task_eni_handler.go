...
func (attachENIHandler *attachTaskENIHandler) handlerFunc() func(message *ecsacs.AttachTaskNetworkInterfacesMessage) {
...
func (attachENIHandler *attachTaskENIHandler) start() {
	go attachENIHandler.handleMessages()
}

// stop is used to invoke a cancellation function
func (attachENIHandler *attachTaskENIHandler) stop() {
	attachENIHandler.cancel()
}

// handleMessages handles each message one at a time
func (attachENIHandler *attachTaskENIHandler) handleMessages() {
	for {
		select {
...
func (handler *attachTaskENIHandler) handleSingleMessage(message *ecsacs.AttachTaskNetworkInterfacesMessage) error {
	receivedAt := time.Now()
