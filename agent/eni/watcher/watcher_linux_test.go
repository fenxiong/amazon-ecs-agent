...
// TestSendENIStateChangeWithAttachmentTypeENI tests that we send the attachment state change
// of a trunk eni as an attachment state change
func TestSendENIStateChangeWithAttachmentTypeTrunkENI(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
...
func TestSendENIStateChangeWithAttachmentTypeENI(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	...
	mockStateManager.EXPECT().ENIByMac(randomMAC).Return(&apieni.ENIAttachment{
		AttachmentType: apieni.ENIAttachmentTypeENI,
		ExpiresAt:      time.Unix(time.Now().Unix()+10, 0),
	}, true)
