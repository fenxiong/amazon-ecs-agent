...
func (mtask *managedTask) handleResourceStateChange(resChange resourceStateChange) {
	...
	} else { //  Failed to clean up resource.
		// When failing to clean up a resource, print an error log and assume it's removed, so its dependency can
		// move forward (e.g. pause container depends on efs volume resource to be removed before moving to stopped).
		// This doesn't seem to be an ideal way, but seems like we don't deal with failure in task resource cleanup
		// ever since task resource is added (refer to deleteTask in docker_task_engine.go), so it is still the case here.
		// Another note: we don't need to mark task desired status to stopped here because we will only try to
		// clean up the resource when we try to stop the task (i.e. task desired status is already STOPPED).
		seelog.Warnf("Managed task [%s]: failed to clean up task resource [%s]: %v", mtask.Arn, res.GetName(), err)
		seelog.Infof("Managed task [%s]: marking resource [%s] known status to REMOVED directly", mtask.Arn, res.GetName())
