...
func TestPortForwardDebug(t *testing.T) {
	taskEngine, done, _ := setupWithDefaultConfig(t)
	defer done()
	...
	t.Logf("Trying to dail ip %s, port %d, timeout %v", cip, port, dialTimeout)
	conn, err := net.DialTimeout("tcp", fmt.Sprintf("%s:%d", cip, port), dialTimeout)
